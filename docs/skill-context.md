...
- 下一次真实用户请求前，context hook 按 session 时间线注入 `<selected_skill>` user-role synthetic message。
- skill body 不进入 system prompt；`/system` 只显示固定 `<skill_policy>`，不显示 skill name、path 或 description。
...
- 追加 `<skill_state status="inactive">`。
- 保留旧 `<selected_skill>` body，保护 llama.cpp / prompt cache 的稳定前缀。
- 后续模型应知道该 skill inactive，不再应用。

`/skill clear --hard`：

- 允许后续上下文物理省略旧 skill body。
- 不注入 unload block，减少后续上下文 token。
...
如果连续 load/clear 之间没有真实会话消息，context hook 会只注入该段结束时的净效果。例如 load → clear → load 只产生最后一个 `<selected_skill>`；load → clear 不产生 skill block。

lazy clear 追加 inactive state，旧 body 仍留在上下文前缀中；hard clear 只省略旧 body，不注入 unload block。
