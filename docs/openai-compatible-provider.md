...
扩展启动时只读取本地缓存，不等待网络。TUI/RPC session 连续空闲 2 秒后，扩展才会在后台并发请求所有 provider 的 models endpoint；`turn_start` 会取消尚未开始的刷新，`turn_end` 再重新安排。成功结果原子写入缓存并更新 Pi model registry，单个 provider 失败时继续使用它的旧缓存。`print` 和 `json` 模式不自动联网刷新。
...
`/refresh-models` 不等待空闲期，会立即刷新；若已有自动刷新正在进行则复用同一请求。`PI_OFFLINE=1` 或 `--offline` 会跳过自动和手动刷新，只使用手写模型与现有缓存。
